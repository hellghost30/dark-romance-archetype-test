...
export default function HomePageEN() {
  ...
  const handleStart = () => {
    // ✅ Don’t allow start without selection
    if (!partnerGender) return;

    if (session) {
      router.push('/en/test');
    } else {
      // ✅ After login redirect to /en/test (selection is already saved in localStorage)
      signIn('google', { callbackUrl: '/en/test' });
    }
  };
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center bg-gray-900 text-white p-8">
      <div className="text-center max-w-2xl w-full">
        <h1
          className="text-4xl md:text-6xl font-serif font-bold text-red-300"
          style={{ color: '#E9D5D5' }}
        >
        ...
        <p className="mt-4 text-lg text-gray-300">
          Take the test and discover which dark archetype from books and movies fits you best.
        </p>

        {/* ✅ Partner selection BEFORE start */}
        <div className="mt-8 w-full max-w-md mx-auto bg-gray-800 rounded-xl p-6 shadow-2xl">
          <h2 className="text-2xl font-serif mb-2">Who are you looking for?</h2>
          <p className="text-gray-300 mb-5 text-sm">
            This affects which archetype category your result will be picked from.
          </p>
          ...
          {partnerGender && (
            <p className="mt-4 text-xs text-gray-400">
              Selected: {partnerGender === 'male' ? 'male' : 'female'}
            </p>
          )}
        </div>
        ...
        >
          {session ? "Start the test" : "Sign in & Start the test"}
        </button>

        {!partnerGender && (
          <p className="mt-3 text-sm text-gray-400">
            Please choose who you’re looking for first.
          </p>
        )}
