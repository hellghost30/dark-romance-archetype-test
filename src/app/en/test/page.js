...
export default function TestPageEN() {
  ...
  // partner gender from localStorage only
  ...
  // If no partnerGender -> go back to EN home (selection happens there)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (status === 'authenticated') {
      fetch('/api/user')
        ...
        .catch(() => {
          // if API fails — block only non-bypass users
          setIsPremium(false);
  ...
  const handleAnswer = async (answer) => {
    ...
    // Increment freeAttemptsUsed only if:
    // - not bypass
    // - not premium
    if (!isBypassUser && !isPremium) {
      try {
        ...
        // even if failed — still show result
      }
    }
    ...
    router.push(`/en/result?${qs}`);
  };

  // If partnerGender not loaded yet — show loader
  if (!partnerGender) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-gray-900">
        <p className="text-white text-xl">Preparing the test...</p>
      </main>
    );
  }
  ...
  if (showPaywall) {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center bg-gray-900 text-white p-8 text-center">
        <h1 className="text-4xl font-serif">You’ve already used your free attempt</h1>
        <p className="mt-4 text-lg text-gray-300">
          Purchase premium to unlock unlimited generations.
        </p>
        ...
        <button
          onClick={async () => {
            try {
              ...
              // Build form and redirect to LiqPay
              ...
              alert("Failed to create payment. Check /api/liqpay/checkout and env vars on Render.");
            }
          }}
          className="mt-8 px-8 py-3 bg-red-800 hover:bg-red-700 text-white font-bold rounded-lg text-xl"
        >
          Buy Premium ({PRICE_UAH} UAH)
        </button>

        <p className="mt-2 text-sm text-gray-500">(Payment page is in progress)</p>
      </main>
    );
  }
    ...
    <main className="flex min-h-screen flex-col items-center justify-center bg-gray-900 text-white p-4 md:p-8">
      <div className="w-full max-w-2xl mb-4">
        <div className="flex items-center justify-between mb-2 text-sm text-gray-400">
          <span>Partner: {partnerGender === 'male' ? 'male' : 'female'}</span>
        </div>
